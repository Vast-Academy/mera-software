...
import { Clock, Eye, UserPlus, MessageCircle, CheckCircle, XCircle, Copy } from 'lucide-react';
...
const copyInstructionsToClipboard = () => {
  if (!selectedRequest || !selectedRequest.instructions || selectedRequest.instructions.length === 0) {
    toast.info('No instructions to copy');
    return;
  }
  
  // Combine all instructions into a single text
  const allInstructions = selectedRequest.instructions
...
<div className="mb-6">
  <div className="flex justify-between items-center mb-2">
    <h4 className="font-medium">Client Instructions</h4>
    {selectedRequest.instructions && selectedRequest.instructions.length > 0 && (
      <button
        onClick={copyInstructionsToClipboard}
        className="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded hover:bg-gray-200 flex items-center"
